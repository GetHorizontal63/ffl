                        ...
                        if (team.roster) {
                            team.roster.forEach(player => {
                                if (player.proTeam) {
                                    ...
                                    const playerTeamAbbr = player.proTeam.toLowerCase();
                                    
                                    // Handle Washington team specifically - check if this is a Washington player
                                    const isWashingtonPlayer = playerTeamAbbr === 'wsh';
                                    
                                    if (isWashingtonPlayer) {
                                        console.log(`Washington player found: ${player.name} (${player.proTeam})`);
                                    }
                                    
                                    // Look for games where this player's team played
                                    let teamGame = null;
                                    
                                    // For Washington players, specifically check for games with "was" team abbreviation
                                    if (isWashingtonPlayer) {
                                        teamGame = nflGames.find(game => {
                                            // Check if the game has Washington as either team
                                            const hasWashingtonTeam = game.teamAbbr?.toLowerCase() === 'was' ||
                                                game.team?.toLowerCase().includes('wash') ||
                                                game.team?.toLowerCase().includes('was') ||
                                                game.team?.toLowerCase().includes('wsh');
                                                
                                            const hasWashingtonOpponent = game.opponent?.toLowerCase().includes('wash') ||
                                                game.opponent?.toLowerCase().includes('was') ||
                                                game.opponent?.toLowerCase().includes('wsh');
                                                
                                            return hasWashingtonTeam || hasWashingtonOpponent;
                                        });
                                        
                                        if (teamGame) {
                                            console.log(`Found game for Washington player ${player.name}:`, teamGame);
                                        } else {
                                            console.log(`No game found for Washington player ${player.name}`);
                                        }
                                    } else {
                                        // Standard matching for non-Washington players
                                        teamGame = nflGames.find(game => {
                                            // Check team abbreviation match
                                            if (game.teamAbbr && game.teamAbbr.toLowerCase() === playerTeamAbbr) {
                                                return true;
                                            }
                                            
                                            // Check team name includes
                                            if (game.team && game.team.toLowerCase().includes(playerTeamAbbr)) {
                                                return true;
                                            }
                                            
                                            // Check opponent name includes
                                            if (game.opponent && game.opponent.toLowerCase().includes(playerTeamAbbr)) {
                                                return true;
                                            }
                                    ...
                                    if (teamGame) {
                                        // Determine if player is on the main team or opponent team
                                        let isMainTeam = false;
                                        
                                        if (isWashingtonPlayer) {
                                            // For Washington, check both was/wsh variations
                                            isMainTeam = teamGame.teamAbbr?.toLowerCase() === 'was' ||
                                                teamGame.team?.toLowerCase().includes('wash') ||
                                                teamGame.team?.toLowerCase().includes('was') ||
                                                teamGame.team?.toLowerCase().includes('wsh');
                                        } else {
                                            // For other teams, standard check
                                            isMainTeam = teamGame.teamAbbr?.toLowerCase() === playerTeamAbbr ||
                                                (teamGame.team && teamGame.team.toLowerCase().includes(playerTeamAbbr));
                                        }
                                        
                                        if (isMainTeam) {
                                            // Player's team is the main team in this game record
                                            player.proOpponent = teamGame.opponent;
                                            player.gameOutcome = teamGame.outcome; // Already W/L/T
                                        } else {
                                            // Player's team is the opponent in this game record
                                            player.proOpponent = teamGame.team;
                                            
                                            // Reverse the outcome for the opponent's perspective
                                            if (teamGame.outcome === 'W') {
                                                player.gameOutcome = 'L';
                                            } else if (teamGame.outcome === 'L') {
                                                player.gameOutcome = 'W';
                                            } else {
                                                player.gameOutcome = 'T';
                                            }
                                        }
                                        
                                        if (isWashingtonPlayer) {
                                            console.log(`Updated Washington player ${player.name} with opponent: ${player.proOpponent}`);
                                        }
                                    }
                                }
                            });
...
async function loadNFLGameData(season, week) {
    try {
        ...
        let response = await fetch(`../data/nfl_games/${season}_week${week}.json`);
        ...
        console.log(`Loaded NFL games for ${season} week ${week}:`, games);
        return games;
    } catch (error) {
            ...
            rosterHTML += `
                <tr>
                    ...
                    <td class="${homePlayer && homePlayer.gameOutcome ? homePlayer.gameOutcome.toLowerCase() : ''}">
                        ${homePlayer && homePlayer.gameOutcome ? homePlayer.gameOutcome : '-'}
                    </td>
                    ...
                    <td class="${awayPlayer && awayPlayer.gameOutcome ? awayPlayer.gameOutcome.toLowerCase() : ''}">
                        ${awayPlayer && awayPlayer.gameOutcome ? awayPlayer.gameOutcome : '-'}
                    </td>
...
function renderPlayerPerformances(game) {
    ...
    if (teamRoster) {
        const teamPlayers = teamRoster
            .filter(p => p.slotPosition !== 'BE' && p.actualPoints) // Only include starting players
            .map(p => ({
                name: p.name,
                position: p.position,
                points: p.actualPoints || 0,
...
function calculateRosterOptimization(roster, rosterRules, season) {
    ...
    roster.forEach(player => {
        // Skip players with no actual points data
        if (!player.actualPoints) {
